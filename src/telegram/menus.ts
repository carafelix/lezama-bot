...
const landingMenu = new Menu<Lezama>("landing")
	.text(async (c) => {
		...
		if (!session.subscribed) {
			...
			await c.reply(
				`Welcome to the Paradiso. Tus poemas quedaron programados para las ${displayHour <= 0 ? displayHour + 24 : displayHour
				}:00, UTC${session.cron.timezoneOffset >= 0
...
const getSelectSubscribeHourText = async (c: Lezama) => {
	const session = await c.session;
	const displayHour = session.cron.hour + session.cron.timezoneOffset;
	return `Selecciona la hora a la que quieres recibir el diario placer. Actual: ${displayHour <= 0 ? displayHour + 24 : displayHour
		}:00, UTC${session.cron.timezoneOffset >= 0
			? "+" + session.cron.timezoneOffset
			: "" + session.cron.timezoneOffset
		}.`;
